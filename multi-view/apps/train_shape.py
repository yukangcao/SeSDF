...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            surface_tensor = train_data['surface'].to(device=cuda).requires_grad_()
            smpl_tensor = train_data['smpl'].to(device=cuda)
            smpl_face_tensor = train_data['smpl_face'].to(device=cuda)
            smpl_normal_tensor = train_data['smpl_normal'].to(device=cuda)
            depth_sample_tensor = train_data['depth_samples'].to(device=cuda)
            depth_surface_tensor = train_data['depth_surface'].to(device=cuda)
            ...
            if opt.num_views > 1:
                smpl_tensor = smpl_tensor.squeeze(2)
                smpl_tensor = smpl_tensor.reshape(smpl_tensor.shape[0]*smpl_tensor.shape[1], smpl_tensor.shape[2], smpl_tensor.shape[3])
                sample_tensor = reshape_sample_tensor(sample_tensor, opt.num_views)
                surface_tensor = reshape_sample_tensor(surface_tensor, opt.num_views)
            
            label_tensor = train_data['labels'].to(device=cuda)
            normal_tensor = train_data['normals'].to(device=cuda)
            sdf_tensor = train_data['sample_sdf'].to(device=cuda)
            ####
            torch.cuda.empty_cache()
            if len(opt.gpu_ids.split(",")) > 1:
                res, error = netG_dp.forward(image_tensor, sample_tensor, surface_tensor, calib_tensor, smpl_vertex=smpl_tensor, smpl_face=smpl_face_tensor, smpl_normal=smpl_normal_tensor, depth_sample=depth_sample_tensor, depth_surface=depth_surface_tensor, labels=label_tensor, normals=normal_tensor, sdf=sdf_tensor, view_id=view_id_tensor)
            else:
                res, error = netG.forward(image_tensor, sample_tensor, surface_tensor, calib_tensor, smpl_vertex=smpl_tensor, smpl_face=smpl_face_tensor, smpl_normal=smpl_normal_tensor, depth_sample=depth_sample_tensor, depth_surface=depth_surface_tensor, labels=label_tensor, normals=normal_tensor, sdf=sdf_tensor, view_id=view_id_tensor)
