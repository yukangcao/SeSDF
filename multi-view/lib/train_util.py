...
def gen_mesh(opt, netG, cuda, data, save_path, use_octree=True):
    ...
    smpl_vertex_norm = normalize_3d_coordinate(smpl_vertices_norm.squeeze(0).permute(0, 2, 1), padding=0.1)
    netG.filter_smpl(smpl_vertex_norm.float())
    ...
    try:
        ...
        verts, faces, _, _ = reconstruction(
            netG, cuda, calib_tensor, smpl_tensor, smpl_vertex_norm, smpl_face_tensor, smpl_normal_tensor, opt.resolution, b_min, b_max, py_min, py_max, use_octree=use_octree)
...
def calc_error(opt, net, cuda, dataset, num_tests):
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(num_tests)):
            ...
            smpl_tensor = data['smpl'].to(device=cuda).unsqueeze(0)
            smpl_face_tensor = data['smpl_face'].to(device=cuda).unsqueeze(0)
            smpl_normal_tensor = data['smpl_normal'].to(device=cuda).unsqueeze(0)
            depth_sample_tensor = data['depth_samples'].to(device=cuda).unsqueeze(0)
            depth_surface_tensor = data['depth_surface'].to(device=cuda).unsqueeze(0)
