...
class SurfaceClassifier(nn.Module):
    ...
    def forward0(self, feature):
        y = feature
        tmpy = feature
        
        y = self._modules['conv'+str(0)](y)
        y = F.leaky_relu(y)
        
        y = self._modules['conv'+str(1)](torch.cat([y, tmpy], 1))
        y = F.leaky_relu(y)
        
        y = self._modules['conv'+str(2)](torch.cat([y, tmpy], 1))
        y = F.leaky_relu(y)
                
        # y = self._modules['conv'+str(3)](torch.cat([y, tmpy], 1))
        # y = F.leaky_relu(y)
        
        return y
        
    def forward1(self, mean, feature_mean):
        y = mean
        tmpy = feature_mean
        y = self._modules['conv'+str(3)](torch.cat([y, tmpy], 1))
        sdf = self.last_op(y)
