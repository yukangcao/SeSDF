...
#        self.mesh_dic = load_trimesh(self.OBJ)
    ...
    def get_render_shape(self, subject, res, num_views, yid=0, pid=0, random_sample=False):
        '''
        ...
        :return:
            ...
            'view_id': [num_views, 1, 1] rotation matrix for rotating the 3D points
        '''
        ...
        for vid in view_ids:
            ...
            # smpl
            # smpl_path = os.path.join(self.SMPLX, subject, '%d_%d_%02d_smplx.obj' % (vid, pitch, 0))
            ...
            smpl_vertex = torch.tensor(smpl_obj.vertices).unsqueeze(0).float()
            smpl_face = torch.tensor(smpl_obj.faces)
            smpl_depth = smpl_obj.vertices[:, 2:3]
            ...
            samples_list = res['samples'].permute(1, 0)
            surface_list = res['surface'].permute(1, 0)
            
            ray_dirs_samples = np.zeros_like(samples_list)
            ray_dirs_samples[:, 2] = -1.0
            ray_dirs_surface = np.zeros_like(surface_list)
            ray_dirs_surface[:, 2] = -1.0
            
            hit_face_samples = rmi.intersects_first(ray_origins=samples_list, ray_directions=ray_dirs_samples)
            hit_face_surface = rmi.intersects_first(ray_origins=surface_list, ray_directions=ray_dirs_surface)
            
            hit_vertices_samples = smpl_face[hit_face_samples, :]
            hit_vertices_surface = smpl_face[hit_face_surface, :]
            
            hit_depth_samples = torch.from_numpy(smpl_depth[hit_vertices_samples[:, 0], 0])
            hit_depth_surface = torch.from_numpy(smpl_depth[hit_vertices_surface[:, 0], 0])
            
            depth_sample_list.append(hit_depth_samples)
            depth_surface_list.append(hit_depth_surface)
        ...
        return {
            ...
            'depth_samples': torch.stack(depth_sample_list, dim=0),
            'depth_surface': torch.stack(depth_surface_list, dim=0)
        }
...
#        exit()
    ...
    def get_item(self, index):
        ...
        if self.opt.num_sample_inout:
            ...
            render_data = self.get_render_shape(subject, res, num_views=self.num_views, yid=yid, pid=pid, random_sample=self.opt.random_multiview)
