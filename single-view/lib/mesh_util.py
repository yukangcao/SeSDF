...
def save_ply(mesh_path, verts):
    file = open(mesh_path, 'w')
    for v in verts:
        file.write('v %.8f %.8f %.8f\n' % (v[0], v[1], v[2]))
    file.close()
    
def gradient_n(inputs, outputs):
...
def reconstruction(net, cuda, calib_tensor, smpl_tensor, smpl_face_tensor, smpl_normal_tensor,
    ...
    def eval_func(points, py_min, py_max):
        ...
        net.query(samples, calib_tensor, smpl_vertex=smpl_tensor, smpl_face=smpl_face_tensor, smpl_normal=smpl_normal_tensor, transforms=transform, py_min=py_min, py_max=py_max, surface=False)
