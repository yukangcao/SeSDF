...
class Evaluator:
    ...
    def load_image(self, image_path, mask_path, smpl_path):
        ...
        smpl_vertex = torch.from_numpy(smpl_obj.vertices).float()
        smpl_face = torch.from_numpy(smpl_obj.faces)
        smpl_normal = torch.from_numpy(smpl_obj.vertex_normals)
        
        return {
            'name': img_name,
            'img': image.unsqueeze(0),
            'calib': calib.unsqueeze(0),
            'smpl': smpl_vertex.unsqueeze(0),
            'smpl_face': smpl_face.unsqueeze(0),
            'smpl_normal': smpl_face.unsqueeze(0),
            'b_min': B_MIN,
            'b_max': B_MAX
        }
