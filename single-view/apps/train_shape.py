...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            sample_tensor = train_data['samples'].to(device=cuda).requires_grad_()
            surface_tensor = train_data['surface'].to(device=cuda).requires_grad_()
            smpl_tensor = train_data['smpl'].to(device=cuda).squeeze(1).squeeze(1)
...
#            torch.cuda.empty_cache()
            if len(opt.gpu_ids.split(",")) > 1:
                res, error = netG_dp.forward(image_tensor, sample_tensor, surface_tensor, calib_tensor, smpl_vertex=smpl_tensor, smpl_face=smpl_face_tensor, smpl_normal=smpl_normal_tensor, labels=label_tensor, normals=normal_tensor, sdf=sdf_tensor, view_id=view_id_tensor)
            else:
                res, error = netG.forward(image_tensor, sample_tensor, surface_tensor, calib_tensor, smpl_vertex=smpl_tensor, smpl_face=smpl_face_tensor, smpl_normal=smpl_normal_tensor, labels=label_tensor, normals=normal_tensor, sdf=sdf_tensor, view_id=view_id_tensor)
